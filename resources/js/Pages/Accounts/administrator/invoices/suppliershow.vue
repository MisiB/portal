                ...
                <v-card-text>
                        ...
                        <template v-slot:default>
                            ...
                            <tbody>
                                ...
                                <tr class="blue--text"><td colspan="3" >Total Due</td><td>{{invoices[0].currency.name}}{{total}}</td></tr>
                                <tr class="green--text"><td colspan="3" >Total Due</td><td>{{invoices[0].currency.name}}{{receipted}}</td></tr>
                                <tr class="red--text"><td colspan="3" >Total Due</td><td>{{invoices[0].currency.name}}{{balance}}</td></tr>
                 ...
                 <div class="display-1 mt-5 d-flex">
                     ...
                     <v-spacer></v-spacer>
                     <div>{{invoices[0].currency.name}}{{wallet}}</div>
                </div>
                <div v-if="wallet > 0">
                 ...
                 <v-card class="mt-5">
                    ...
                    <template v-slot:default>
                        ...
                        <tbody>
                            <tr v-for="rtg in invoices[0].rtgs" :key="rtg.id" class="pt-2 pb-2">
                              <td>{{rtg.created_at | formatDate}}</td>
                              <td>{{rtg.invoice_number}}</td>
                              <td>{{rtg.name}}</td>
                              <td >
                                 <div v-if="rtg.status=='PENDING'" class="d-flex pa-2 justify-end">
...
export  default {
    ...
    components: {
        ...
        filterData(){
            this.$inertia.put(this.$route('awaitingsupplierinvoices.update',this.id),{term:this.filter,inv:this.invoices[0].id},{
                onSuccess:(page)=>{
        ...
        processRtgs(id){
                    ...
                    if (result.isConfirmed) {
                        this.loading= true
                          var data ={rtgs:this.rtgs_id,id:id}
                                    this.$inertia.post(this.$route('awaitingsupplierinvoices.store'),data,{
                                        onSuccess:(page)=>{
                                           this.loading = false
                ...
                Vue.swal({
                    ...
                    if (result.isConfirmed) {
                        this.loading= true
                        this.$inertia.get(this.$route('awaitingsupplierinvoices.edit',this.invoices[0].invoice_number))                     
