<template>
       <userlayout title="Procurement Classes">
           <v-container>
            <v-card flat>
                <v-card-text>
                    <div class="d-flex">
                        <v-btn text link :href="$route('admin.home')" >Dashboard</v-btn> <v-btn text link disabled>Procurement Classes</v-btn>
                    </div>
                </v-card-text>
            </v-card>               
             <v-card flat class="mt-4">
                 <v-card-title>
                    <div class="display-1">Procurement Classes</div>
                    <v-spacer/>
                      <v-btn
                        class="mx-2"
                        fab
                ...
                <v-card-text>
                    <v-simple-table>
                        <template v-slot:default>
                            ...
                            <tbody>
                                <tr v-for="clss in procurementclasses" :key="clss.id">
                                    <td>
                                        {{clss.name}}
                                    </td>
            ...
            this.form.post(this.$route('procurementclass.store'),{
             ...
             Vue.swal({
                    ...
                    if (result.isConfirmed) {
                        this.loading= true
                        this.$inertia.delete(this.$route('procurementclass.destroy',id),{
                                    onSuccess:(page)=>{
                                    this.loading = false
                                        Vue.swal(
                                            'Deleted!',
