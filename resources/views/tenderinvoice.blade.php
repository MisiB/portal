...
<div class="container-fluid">
    <div style="width: 200px; height:200px; text-align:left; padding-left:5px;">
        ...
        <table class="table table-bordered">
            <tr><th>Number</th><td>{{$invoice[0]->invoice_number}}</td></tr>
            <tr><th>Date:</th><td>{{Carbon\Carbon::now()}}</td></tr>
        </table>
    </div>
    <div style="float:right; width:200px;height:200px; margin-top:-150px; padding-right:5px;">
    ...
    <div style="width:100%; margin-top:100px">
        <table class="table table-bordered">
            <?php $total =0; $curreny='';?>
            <tr><th>Date</th><th>Code</th><th>Description</th><th></th><th class="text-right">Amount</th></tr>
            @forelse($invoice as $invo)
                <tr>
                    <td>{{$invo->created_at}}</td>
                    <td>{{$invo->type}}</td>
                    <td>{{$invo->description}}({{$invo->year}})</td> 
                    <td></td>                   
                    <td class="text-right">{{$invo->currency->name}}{{$invo->amount}}</td>
            ...
            <tr>
                ...
                <td class="text-right">
                    <table class="table table-bordered">
                        <tr><th>Total Due</th><td><b>{{$invoice[0]->currency->name}}{{$invoice->sum('amount')}}</b></td></tr>
                    </table>
                </td>
            </tr>
        </table>
        ...
        <p>
            @if(strtoupper($invoice[0]->currency->name) =='ZWL')
                <h3>Bid Bond Account </h3>
                <b>Account Name : Procurement Regulatory Authority of Zimbabwe</b><br/>
                <b> Bank : CBZ</b><br/>
                <b>Account Number: 01121064850030</b><br/>
                <b>Branch : Kwame Nkrumah</b>

                <h3>Administrative Fee Account </h3>
                <b>Account Name : Procurement Regulatory Authority of Zimbabwe</b><br/>
                <b> Bank : CBZ</b><br/>
